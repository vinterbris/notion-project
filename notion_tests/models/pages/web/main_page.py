...
class MainPage:
    ...
    def __init__(self):
        self.close_modal = browser.element('.closeSmall')
        self.button_choose_default_page_getting_started = browser.all(
            '[role="button"] .notranslate'
        ).element_by(have.text('Getting Started'))
        self.button_choose_peek_mode = browser.element('.peekModeCenter')
        self.subpage_name = browser.element(
    ...
    class TopBar:
        ...
        def __init__(self):
            ...
            self.favorites_header = browser.element(
                '.notion-outliner-bookmarks-header-container'
            )
    ...
    class SideBar:
        def __init__(self):
            self.button_archive_without_moving_pages = browser.all(
                '[role="button"]'
            ).element_by(have.text('Proceed without moving pages'))
            self.button_archive_teamspace = browser.all('[role="button"]').element_by(
                have.text('Archive teamspace')
            )
            self.menu_archive_teamspace = browser.all('[role="menuitem"]').element_by(
                have.text('Archive teamspace')
            )
            self.teamspace_home = browser.all('.notion-outliner-team').element_by(
                have.text('Teamspace Home')
            )
            self.list_of_teamspace_pages = browser.all('.notion-outliner-team')
            self.teamspace_page = browser.element('.notion-outliner-team')
            self.button_teamspace_page_plus = browser.element(
                '.notion-outliner-team-header-container .plusThick'
            )
            self.list_of_buttons = browser.all('[role="button"]')
            self.teamspace_header_container = browser.element(
                '.notion-outliner-team-header-container'
            )
            self.button_trash = browser.element('.newSidebarTrash')
            # self.button_import = browser.element('.sidebarImport')
            self.button_calendar = browser.all('[role="button"]').element_by(
                have.text('Calendar')
            )
            self.block_of_private_pages = browser.element('.notion-outliner-private')
            self.button_new_page = browser.element('.createPage')
            # self.settings = browser.element('.sidebarSettings')
            self.settings = browser.all('[role="button"]').element_by(
                have.exact_text('Settings & members')
            )
            # self.inbox = browser.element('.sidebarInbox')
            self.inbox = browser.all('[role="button"]').element_by(have.exact_text('Inbox'))
            # self.search = browser.element('.sidebarSearch')
            self.search = browser.all('[role="button"]').element_by(have.exact_text('Search'))
            ...
            self.button_skip_people_invite = browser.all('[role="button"]').element_by(
                have.text('Skip for now')
            )
            self.button_submit_teamspace = browser.all('[role="button"]').element_by(
                have.text('Create teamspace')
            )
            self.teamspace_name = browser.element('[placeholder="Acme Labs"]')
            self.button_create_teamspace = browser.all('[role="button"]').element_by(
                have.text('Create a teamspace')
            )
            self.page_button_plus = browser.element(
                '.notion-outliner-private .notion-page-block .plusThick'
            )
        ...
        def should_have_teamspace_ui_elemnts(self, workspace_name):
            ...
            self.list_of_teamspace_pages.element_by(have.text(workspace_name)).should(
                be.present
            )
        ...
        def archive_teamspace(self, workspace_name):
            ...
            self.list_of_teamspace_pages.element_by(have.text(workspace_name)).element(
                '.dots'
            ).click()
