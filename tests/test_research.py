...
def login():
    ...
    if browser.element('[autocomplete="one-time-code"]').matching(be.present):
...
def test_create_page(delete_current_page):
    ...
    # THEN
    browser.element('.shadow-cursor-breadcrumb').should(have.text('Untitled'))
...
def test_add_page_to_favorites(delete_current_page):
    login()

    browser.all('[role="button"]').element_by(have.text('Add a page')).click()
    browser.element('[placeholder="Untitled"]').type('Favorites test')
